...
namespace Week2Oefening1.Models.DAL
{
    public class OrderRepository : GenericRepository<Order>, Week2Oefening1.Models.DAL.IOrderRepository
    {
        ...
        {
            return this.context.Orders.AsNoTracking<Order>().Include(o => o.OrderLines).Include(u => u.User);
        }

        public IEnumerable<Order> AllOfUser(String id)
        {
            var query = from o in this.context.Orders.AsNoTracking<Order>().Include(l => l.OrderLines).Include(u => u.User) where o.User.Id == id select o;
            return query;
        }
