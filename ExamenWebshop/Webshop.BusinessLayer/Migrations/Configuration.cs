namespace Webshop.BusinessLayer.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<Webshop.BusinessLayer.Context.ApplicationDbContext>
    {
        ...
        protected override void Seed(Webshop.BusinessLayer.Context.ApplicationDbContext context)
        {
        ...
        private List<Device> ReadDevices(ApplicationDbContext context)
        {
            ...
            while (line != null)
            {
                ...
                Device device = new Device()
                {
                    ...
                    RentPrice = Convert.ToDouble(parts[3]),
                    Stock = Convert.ToInt32(parts[4]),
                    Picture = parts[5],
                    Description = parts[8],
                    DeviceOSs = GetDeviceOS(context, parts[6]),
                    DeviceFrameworks = GetFrameworkOS(context, parts[7])
                };
                devices.Add(device);
                line = osr.ReadLine();
            }
